...
final class SearchPageController: UIViewController {
    private enum Constants {
        ...
        static let resultCellHeight: CGFloat = 250
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if !suggestionScrollView.isHidden, searchBar.canBecomeFirstResponder {
        ...
        recentsProvider.getRecentMeals { [weak self] result in
            ...
            case let .success(items):
                DispatchQueue.main.async {
                    self.filters = []
                    self.mealsToShow = items.meals.shuffled()
    ...
    private func search(searchText: String) {
        guard !(searchText.isEmpty && filters.isEmpty) else {
            showRecentMeals()
            return
        }
